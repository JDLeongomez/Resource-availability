...
```{r sociodemographic-numeric-desc-plot, message = FALSE, warning = FALSE, results = "hold", fig.cap = "Distribution of values across numeric sociodemographic variables by condition. Colored vertical lines indicate the mean value for each variable under each condition.", fig.height = 8}
# Transform desc_quest by selecting only certain columns and pivoting them into a long format
desc_quest |>
  # Select ID, Condition, and all numeric columns
  select(ID, Condition, where(is.numeric)) |> 
  # Pivot the numeric columns into a long format with "Variable" as the column name
  pivot_longer(where(is.numeric),
               names_to = "Variable",
               values_to = "Value") |>
  # Clean up the Variable names by replacing underscores and "Freq" and "SP" substrings.
  ...
  # Create a ggplot object to visualize the transformed data.
  ggplot(aes(x = Value, fill = Condition, color = Condition)) +
  # Create a density plot for each Variable, with different colors and fills based on Condition
  ...
  # Add a vertical line at the mean Value for each Variable.
  stat_summary(aes(xintercept = after_stat(x), y = 0),
               fun = mean, geom = "vline", orientation = "y") +
  # Divide the plot into facets by Variable, with free scales
...
```{r sociodemographic-categorical-desc-plot, message = FALSE, warning = FALSE, results = "hold", fig.cap = "Proportional number of participants across categorical variables by condition.", fig.height = 9}
# Transform desc_quest by selecting only certain columns and pivoting them into a long format
desc_quest |>
  # Select ID, Condition, all factor variables (will be renamed to Factor), City, and exclude
  # any variables starting with "Escasez alimentaria".
  select(ID, Condition, where(is.factor), City, -starts_with("Escasez alimentaria")) |> 
  # Pivot the long data from City:Victim_of_armed_conflict into a wide format.
  pivot_longer(City:Victim_of_armed_conflict,
               names_to = "Variable",
               values_to = "Value") |>
  # Clean up the Variable names by replacing underscores with spaces
  mutate(Variable = str_replace_all(Variable, "_", " ")) |> 
  # Create a ggplot object to visualize the transformed data
  ggplot(aes(y = Value, fill = Condition, color = Condition)) +
  # Create a bar plot for each Variable, with different colors and fills based on Condition
  geom_bar(alpha = 0.3, position = position_dodge()) +
  # Add text labels to show the proportion of each category within each group
  ...
  # Divide the plot into facets by Variable, with free scales 
  facet_wrap(~Variable, scales = "free") +
  # Set up scale for y-axis to display counts in a compact manner
  scale_y_discrete(labels = label_wrap(25)) +
  # Adjust the appearance of the plot by making axis text slightly smaller
