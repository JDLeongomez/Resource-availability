...
```{r}
main.eff.contr <- function(model) {
  merge(data.frame(emmeans(model, pairwise ~ Sexual_dimorphism)$emmeans),
        data.frame(emmeans(model, pairwise ~ Sexual_dimorphism)$contrast) |> 
    ...
    kable(digits = 2, 
          ...
          caption = paste0("Estimated marginal and constrast between masculinized and feminized 
                           stimuli for the ", find_response(model), " model"),
          col.names = c("Sexual dimorphism", 
                        "EMM", 
                        "$SE$", 
                        "$2.5\\%CI$", 
                        "$97.5\\%CI$", 
          ...
          escape = FALSE) |> 
    kable_styling(latex_options = c("HOLD_position", "scale_down")) |>
    add_header_above(c(" " = 5,"Contrast (Masculinized - Feminized)" = 4)) |> 
...
inter.contr <- function(model, emm_contr) {
  ...
  merge(data.frame(emm_contr$emmeans),
    ...
    kable(digits = 2, 
          ...
          linesep ="", 
          caption = paste0("Estimated marginal and constrast between masculinized and feminized 
                           stimuli by ", emm_contr$emmeans@roles$predictors[2], " for the ",
          ...
          escape = FALSE) |> 
    kable_styling(latex_options = c("HOLD_position", "scale_down")) |>
    add_header_above(c(" " = 6,"Contrasts (Masculinized - Feminized)" = 4)) |> 
    collapse_rows(1,
                  latex_hline = "major",
...
full.contr <- function(model, emm_contr) {
  ...
  merge(data.frame(emm_contr$emmeans),
    ...
    kable(digits = 2, 
          ...
          escape = FALSE) |> 
    kable_styling(latex_options = c("HOLD_position", "scale_down")) |>
    add_header_above(c(" " = 7,"Contrasts (Masculinized - Feminized)" = 4)) |> 
...
#### Differences in fixations to masculinized and feminized stimuli
...
dat_dif <- dat |> 
  ...
  summarise(DFF_dif = DFF[Sexual_dimorphism == "Masculinized"] - 
              DFF[Sexual_dimorphism == "Feminized"],
            TFD_dif = TFD[Sexual_dimorphism == "Masculinized"] - 
              TFD[Sexual_dimorphism == "Feminized"],
            NF_dif = NF[Sexual_dimorphism == "Masculinized"] - 
              NF[Sexual_dimorphism == "Feminized"],
            Attr_dif = Attractiveness[Sexual_dimorphism == "Masculinized"] - 
              Attractiveness[Sexual_dimorphism == "Feminized"],
            Masc_dif = Masculinity[Sexual_dimorphism == "Masculinized"] - 
              Masculinity[Sexual_dimorphism == "Feminized"]) |> 
  ungroup() |> 
  mutate(across(where(is.character), as.factor))
```
...
### Correlations between partner perceptions
...
quests_fin |>
  select(
    ...
    Partner_attractiveness) |>
  rename(
    "Physical violence" = "Freq_partner_physical_violence",
    "Sexual violence" = "Freq_partner_sexual_violence",
    "Infidelity" = "Freq_partner_infidelity",
  ...
  kable(digits = 2,
        ...
        align = c("l", rep("c", 5)),
        linesep = "",
        caption = "Correlations between partner perceptions",
        escape = FALSE) |>
  kable_styling(latex_options = c("HOLD_position")) |>
  footnote(general = paste0("Values represent Pearson correlation coefficients ($r$). ",
                            "Physical violence, sexual violence, and infidelity are frequencies. ",
...
desc_quest |> 
  ...
  add_header_above(c(" ", 
                   ...
                   bold = TRUE) |> 
  footnote(general = paste0("Values represent Pearson correlation coefficients ($r$). ",
                            "S.P. = self-perceived; masc. = masculinity; 
...
### Correlations between fixations, subjective evaluations of stimuli, partner perceptions, and violence
...
dat_main_corr_high <- dat_main_corr |> 
...
dat_main_corr_low <- dat_main_corr |> 
  filter(Condition == "High") |> 
  ungroup() |> 
  select(TFD:Freq_partner_infidelity) |> 
  rename_with(~str_replace_all(., "_", " ")) |> 
...
dat_main_corr_high |> 
  bind_rows(dat_main_corr_low) |> 
  kable(digits = 2,
        booktabs = TRUE,
        align = c("l", rep("c", 20)),
        linesep = "",
        caption = "Correlations between fixations, subjective evaluations of stimuli, 
        partner perceptions, and violence by Condition",
        escape = FALSE) |>
  kable_styling(latex_options = c("HOLD_position"),
                font_size = 4.2) |>
  column_spec(1, width = "2.5cm") |>
  column_spec(2:21, width = "0.84cm") |>
  add_header_above(c(" ", 
                     "Fixations" = 3, 
                     "Stimulus ratings" = 2,
                     "Current/last partner\nperception" = 3,
                     ...
                     "Frequency of\npartner violence" = 2),
                   bold = TRUE) |> 
  pack_rows("Condition: High", 1, 17,
            hline_after = TRUE) |> 
  pack_rows("Condition: Low", 18, 34,
            hline_after = TRUE, hline_before = TRUE) |> 
  footnote(general = paste0("Values represent Pearson correlation coefficients ($r$). ",
                            "TDF = Total Fixation Duration; DFF = Duration of First Fixation;
...
### Correlations between partner violence and mean difference scores between responses to masculinized and feminized stimuli
...
dat_dif_short |> 
  ...
  kable(digits = 2,
        ...
        align = c("l", rep("c", 10)),
        linesep = "",
        caption = "Correlations between partner violence and mean difference scores between 
        responses to masculinized and feminized stimuli",
        escape = FALSE) |>
  kable_styling(latex_options = c("HOLD_position", "scale_down")) |>
  add_header_above(c(" ",
                     " " = 4, 
                     "Difference scores\n(masculinized - feminized)" = 5),
                   bold = TRUE) |>
  pack_rows("Low condition, short-term relationship", 1, 9,
            hline_after = TRUE) |> 
  pack_rows("Low condition, long-term relationship", 10, 18,
            hline_after = TRUE, hline_before = TRUE) |>
  pack_rows("High condition, short-term relationship", 19, 27,
            hline_after = TRUE, hline_before = TRUE) |> 
  pack_rows("High condition, long-term relationship", 28, 36,
            hline_after = TRUE, hline_before = TRUE) |> 
  footnote(general = paste0("Values represent Pearson correlation coefficients ($r$). ",
                            ...
                            attr. = attractiveness; 
                            masc. = masculinity. ", 
                            "For variables that were evaluated in response to each stimulus 
                            ...
                            a stronger preference for masculinized stimuli. ",
                            "For significance, $^{\\\\dagger}p$ < 0.1, *$p$ < 0.05, ",
                            "**$p$ < 0.01, ***$p$ < 0.001. ",
                            "Significant correlations are in bold."),
           threeparttable = TRUE,
...
reg_fin |> 
  ...
  summarise("Mean (Low)" = mean(Score[reg_fin$Condition == "Low"]),
            ...
            "\\textit{t}" = t.test(Score ~ Condition)$statistic,
            "\\textit{p}" = t.test(Score ~ Condition)$p.value,
            "\\textit{g}" = hedges_g(Score ~ Condition)$Hedges_g) |> 
  ungroup() |> 
  mutate(Dimension = str_replace_all(Dimension, "_", " "),
         Dimension = str_remove_all(Dimension, "Condition "),
         Dimension = str_to_title(Dimension)) |> 
  mutate("\\textit{p}" = pval.lev("\\textit{p}")) |> 
  kable(digits = 2,
        booktabs = TRUE,
        align = c("l", rep("c", 5)),
        linesep = "",
...
# Models of the experimental design
...
```{r assu-mod1, cache = TRUE, message = FALSE, fig.height = 11, fig.cap = "Model assumptions. Plots represent prediction check, linearity, homogeneity of variance, influential observations, collinearity, and normality of both residuals and random effects (as QQ plots), respectively."}
check_model(mod1)
```
...
pm1 <- plot.exp(model = mod1, y.pos = c(266, NA, NA, 268))
pm1
```
...
```{r}
inter.contr(model = mod1,
            emm_contr = emmeans(mod1, pairwise ~ Sexual_dimorphism | Relationship))

```

```{r fig-mod1X, results = "hold", fig.cap = "Differences in the XXXXXX (XXX) to masculinized and feminized stimuli, by relationship (short term, ling term) and condition (low, high). Black dots and bars represent estimated marginal means ± standard errors. For detailed results, see Tables \\@ref(tab:tab-modX) and \\@ref(tab:tab-contr-modX)). In all cases, significant effects are represented with lines and stars: \\**p* < 0.05, \\*\\**p* < 0.01, \\*\\*\\**p* < 0.001, \\*\\*\\*\\**p* < 0.0001.", warning = FALSE, message = FALSE}
pm2 <- plot.exp(model = mod1, y.pos = c(266, NA, NA, 268))
pm2
```
