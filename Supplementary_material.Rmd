...
author:
  - name: Milena Vásquez-Amézquita \orcidlink{0000-0001-7317-8430}
    correspondence: true
    institute: [labpsiexp, costa]
    email: mvasquezam@unbosque.edu.co
  - name: Andrés Castellanos-Chacón \orcidlink{0000-0003-1684-9319}
    correspondence: false
    institute: evoco
  - name: Wendy Medina-Sarmiento \orcidlink{0000-0001-9695-4703}
    correspondence: false
    institute: costa
  - name: Valentina Cepeda \orcidlink{0009-0007-2620-6743}
    correspondence: false
    institute: labpsiexp
  ...
  - name: Juan David Leongómez \orcidlink{0000-0002-0092-6298}
    ...
    institute: evoco
institute:
  - labpsiexp: "Experimental Psychology Lab, Faculty of Psychology, Universidad El Bosque, Bogota, Colombia"
...
date: "`r if (.Platform$OS.type == 'windows') { Sys.setlocale('LC_TIME','English') } else { Sys.setlocale('LC_TIME','en_US.UTF-8') }; format(Sys.Date(), '%B %d, %Y')`"
output:
  bookdown::pdf_document2:
    ...
    toc: no
    pandoc_args:
      - '--lua-filter=lua/scholarly-metadata-letters.lua'
      - '--lua-filter=lua/author-info-blocks-letters.lua'
classoption: 
      - bookmarksnumbered
editor_options:
...
Vásquez-Amézquita, M., Castellanos-Chacón, A., Medina-Sarmiento, W., Cepeda, V., Martínez-González, M. B., \& Leongómez, J. D.,  (in press). Resource availability and experiences of partner violence shape facial masculinity preferences in Colombian women. \textit{Evolution and Human Behavior}.
\end{hangparas}
```
...
### Effects of sexual dimorphism manipulation on perceived age

Lastly, we tested whether perceived age ratings given to the masculinized and feminized versions of each stimuli differed. This was in fact the case for all stimuli; in all cases masculinized versions were rated as significantly older than feminized versions.

```{r}
# Group by stimulus and perform t-tests for age ratings across
# sexual dimorphism categories
t_age <- age_dat |>
  group_by(Stimulus) |>
  summarise(
    t = round(t.test(Age ~ Sexual_dimorphism)$statistic, 2), # Compute t values
    p = pval.lev(t.test(Age ~ Sexual_dimorphism)$p.value)
  ) |> # Compute p-value
  ungroup()

# Select the first 10 rows of the data 't_age'
t_age[1:10, ] |>
  # Add the next 10 rows (11 to 20) as additional columns
  cbind(t_age[11:20, ]) |>
  # Add the next 10 rows (21 to 30) as additional columns
  ...
  kable(
    ...
    align = "c", # Center align all columns
    linesep = "", # No lines between rows
    caption = "Difference in independent age ratings given to each stimulus,
               according to its sexual dimorphism manipulation (feminized - masculinized)",
    # Caption for the table
    escape = FALSE, # Allow LaTeX commands in the table (e.g., italic or bold)
    col.names = rep(c("Stimulus", "$t$", "$p$"), times = 3) # Column names
...
```{r}
quests <- read_excel(
  "Data/Cuestionario Datos Sociodemográficos  (Disponibilidad) (respuestas) (1).xlsx",
...
When including all socio-ecological factors, the only variables that strongly correlate between them and with the PCA dimension (Table \@ref(tab:pca-sef-tab); Fig. \@ref(fig:pca-sef-plot)), are the two variables that evaluate participant's perception of men as dangerous to children and to their partner.
...
tab_m1$tab |> 
  ...
  distribution in the case of NF); for PCF, results are from a linear model without 
...
tab_m1_cov$tab |> 
  ...
  add_header_above(c(" " = 1,
                     ...
                     "Number of Fixations (NF)" = 3,
                     "Proportion of Chosen Faces (PCF)" = 4)) |> 
  footnote(general = "Best-supported model for predicting each dependent variable. 
