...
\subsection{Sociodemographic data}\label{sociodemographic-data}

\subsubsection{Select and wrangle data}\label{select-and-wrangle-data}
...
\subsubsection{Sociodemographic numeric}\label{sociodemographic-numeric}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Transform desc\_quest by selecting only certain columns and pivoting them into a long format}
\NormalTok{desc\_quest }\SpecialCharTok{|\textgreater{}}
  \CommentTok{\# Select ID, Condition, and all numeric columns}
  \FunctionTok{select}\NormalTok{(ID, Condition, }\FunctionTok{where}\NormalTok{(is.numeric)) }\SpecialCharTok{|\textgreater{}} 
  \CommentTok{\# Pivot the numeric columns into a long format with "Variable" as the column name}
  \FunctionTok{pivot\_longer}\NormalTok{(}\FunctionTok{where}\NormalTok{(is.numeric),}
               \AttributeTok{names\_to =} \StringTok{"Variable"}\NormalTok{,}
               \AttributeTok{values\_to =} \StringTok{"Value"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
  \CommentTok{\# Clean up the Variable names by replacing underscores and "Freq" and "SP" substrings.}
  ...
  \CommentTok{\# Create a ggplot object to visualize the transformed data.}
  ...
  \CommentTok{\# Create a density plot for each Variable, with different colors and fills based on Condition}
  \FunctionTok{geom\_density}\NormalTok{(}\AttributeTok{alpha =} \FloatTok{0.3}\NormalTok{) }\SpecialCharTok{+}
  \CommentTok{\# Set up plot labels and title.}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \ConstantTok{NULL}\NormalTok{, }\AttributeTok{y =} \StringTok{"Density"}\NormalTok{) }\SpecialCharTok{+}
  ...
  \CommentTok{\# Add a vertical line at the mean Value for each Variable.}
  ...
  \CommentTok{\# Divide the plot into facets by Variable, with free scales}
  \FunctionTok{facet\_wrap}\NormalTok{(}\SpecialCharTok{\textasciitilde{}}\NormalTok{Variable, }\AttributeTok{scales =} \StringTok{"free"}\NormalTok{,}
             \AttributeTok{labeller =} \FunctionTok{labeller}\NormalTok{(}\AttributeTok{Variable =} \FunctionTok{label\_wrap\_gen}\NormalTok{(}\AttributeTok{width =} \DecValTok{25}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}

\begin{figure}
\centering
\includegraphics{Supplementary_material_files/figure-latex/sociodemographic-numeric-desc-plot-1.pdf}
\caption{\label{fig:sociodemographic-numeric-desc-plot}Distribution of values across numeric sociodemographic variables by condition. Colored vertical lines indicate the mean value for each variable under each condition.}
\end{figure}

\subsubsection{Sociodemographic categorical}\label{sociodemographic-categorical}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Transform desc\_quest by selecting only certain columns and pivoting them into a long format}
\NormalTok{desc\_quest }\SpecialCharTok{|\textgreater{}}
  \CommentTok{\# Select ID, Condition, all factor variables (will be renamed to Factor), City, and exclude}
  \CommentTok{\# any variables starting with "Escasez alimentaria".}
  \FunctionTok{select}\NormalTok{(ID, Condition, }\FunctionTok{where}\NormalTok{(is.factor), City, }\SpecialCharTok{{-}}\FunctionTok{starts\_with}\NormalTok{(}\StringTok{"Escasez alimentaria"}\NormalTok{)) }\SpecialCharTok{|\textgreater{}} 
  \CommentTok{\# Pivot the long data from City:Victim\_of\_armed\_conflict into a wide format.}
  \FunctionTok{pivot\_longer}\NormalTok{(City}\SpecialCharTok{:}\NormalTok{Victim\_of\_armed\_conflict,}
               \AttributeTok{names\_to =} \StringTok{"Variable"}\NormalTok{,}
               \AttributeTok{values\_to =} \StringTok{"Value"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
  \CommentTok{\# Clean up the Variable names by replacing underscores with spaces}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{Variable =} \FunctionTok{str\_replace\_all}\NormalTok{(Variable, }\StringTok{"\_"}\NormalTok{, }\StringTok{" "}\NormalTok{)) }\SpecialCharTok{|\textgreater{}} 
  \CommentTok{\# Create a ggplot object to visualize the transformed data}
  ...
  \CommentTok{\# Create a bar plot for each Variable, with different colors and fills based on Condition}
  ...
  \CommentTok{\# Add text labels to show the proportion of each category within each group}
  ...
  \CommentTok{\# Set up scale for y{-}axis to display counts in a compact manner}
  ...
  \CommentTok{\# Adjust the appearance of the plot by making axis text slightly smaller}
...
\begin{figure}
\centering
\includegraphics{Supplementary_material_files/figure-latex/sociodemographic-categorical-desc-plot-1.pdf}
\caption{\label{fig:sociodemographic-categorical-desc-plot}Proportional number of participants across categorical variables by condition.}
\end{figure}
