...
-- Convert a number to a corresponding letter (e.g., 1 -> 'a', 2 -> 'b', etc.)
local function num_to_letter(num)
  return string.char(96 + num)  -- 96 is the ASCII value before 'a'
end
...
local function resolve_institutes (institute, known_institutes)
  ...
  for i, inst in ipairs(unresolved_institutes) do
    -- Convert index to a letter if 'inst' is a number
    local inst_key = type(inst) == "number" and num_to_letter(inst) or inst

    result[i] =
      known_institutes[inst_key] or
...
local function canonicalize(raw_author, raw_institute)
  ...
  for idx, inst in ipairs(institutes) do
    inst.index = pandoc.MetaInlines{pandoc.Str(num_to_letter(idx))}
  end
